...
## 🔆 Commit 11 - 2021/11/17 03:53

---

XHR을 이용한 동기적 구조 였었던 API Class 부분을 콜백함수를 이용해 비동기로 변경해주었다.

지금은 몇안되는 API여서 콜백함수가 비교적 간단했지만 많아지면 많아질수록 콜백지옥에 빠지기 쉽다.

그래서 Fetch 와 Promise 객체를 이용해서 Depth가 깊어지는게 아닌 방법으로 구현할 수 있었다.

하지만 이 방법도 콜백함수는 여전히 존재한다. Depth가 깊어지지 않는 코드라 가독성이 뛰어나 졌을 뿐

궁극적으로는 콜백함수도 사용하지 않고 비동기로 데이터를 가져오게 하는게 목표이고 이를 위해서는 async await를 사용해야 한다.

> 비동기 파트는 callback 함수를 활용해서 구현하는 것부터 Promise 까지 확실하게 이해 하고 Async Await를 사용해야 한다.

> 그냥 무작정 Async Await가 간단하게 할 수 있다고 사용하다가는 후에 반드시 문제가 생긴다. 반복 학습 하자
